...
struct affine_transform {
  ...
  static affine_transform<Field, Dim> translation(const fixvec<Field, Dim> &t) {
  ...
  static affine_transform<Field, Dim> anisotropic_scaling(Args... args) {
    affine_transform<Field, Dim> result = {fixmat<Field, Dim, Dim>::eye(),
                                           fixvec<Field, Dim>::zero()};

    std::array<Field, Dim> sf = {args...};

    for (size_t i = 0; i < Dim; ++i) result.linear(i, i) = sf[i];
  ...
  static typename std::enable_if<Dim == 3, affine_transform<Field, 3>>::type
  rotation(const fixvec<Field, 3> &axis, const Field &angle) {
    using std::cos;
    using std::sin;
    ...
    result.linear = {x * x * C + c,     x * y * C - z * s, x * z * C + y * s,
  ...
  static typename std::enable_if<Dim == 3, affine_transform<Field, 3>>::type
  basis_mapping(const fixvec<Field, 3> &t1, const fixvec<Field, 3> &t2,
                const fixvec<Field, 3> &t3) {
    affine_transform<Field, 3> result;

    result.linear = {t1(0), t1(1), t1(2), t2(0), t2(1),
                     t2(2), t3(0), t3(1), t3(2)};
...
/// Compose transforms.
template <class Field, size_t Dim>
affine_transform<Field, Dim> operator*(const affine_transform<Field, Dim> &a,
...
template <class Field, size_t Dim>
fixvec<Field, Dim> operator*(const affine_transform<Field, Dim> &a,
...
template <class Field, size_t Dim>
affine_transform<Field, Dim> inverse(const affine_transform<Field, Dim> &t) {
  auto total = fixmat<Field, Dim + 1, Dim + 1>::eye();

  for (size_t i = 0; i < Dim; ++i)
    for (size_t j = 0; j < Dim; ++j) total(i, j) = t.linear(i, j);

  for (size_t i = 0; i < Dim; ++i) total(i, Dim) = t.offset(i);
  ...
  for (size_t i = 0; i < Dim; ++i)
    for (size_t j = 0; j < Dim; ++j) result.linear(i, j) = inv_total(i, j);

  for (size_t i = 0; i < Dim; ++i) result.offset(i) = inv_total(i, Dim);
