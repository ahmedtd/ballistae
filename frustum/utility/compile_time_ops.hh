...
constexpr auto product(ArgsHead head, ArgsSecond second, ArgsTail... tail) {
  return head * product(second, tail...);
}

template <class Arg>
constexpr auto select_ith(size_t i, Arg head) {
  return head;
}

template <class ArgsHead, class ArgsSecond, class... ArgsTail>
constexpr auto select_ith(size_t i, ArgsHead head, ArgsSecond second,
                          ArgsTail... tail) {
  return (i == 0) ? head : select_ith(i - 1, second, tail...);
}
...
struct argpack_to_range {
  ...
  static RangeIt work(RangeIt output_begin, ArgHeadType arg_head,
                      ArgTailTypes... arg_tail) {
    *output_begin = arg_head;
    return argpack_to_range<ArgTailTypes...>::work(output_begin + 1,
                                                   arg_tail...);
  }
};
...
struct argpack_to_range<ArgFinalType> {
  template <class RangeIt>
  static RangeIt work(RangeIt output_begin, ArgFinalType arg_final) {
...
struct integer_sequence_construct<integer_sequence<Int, Seq1...>,
