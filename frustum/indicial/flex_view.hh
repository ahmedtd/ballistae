...
template <class Derived, std::size_t AccessIndex, class... ParamPackTypes>
struct apply_offsets_to_param_pack<Derived, AccessIndex,
                                   std::tuple<ParamPackTypes...> > {
  static std::size_t work(std::vector<std::size_t> offsets,
...
template <class Derived, std::size_t... AccessIndices,
          class... PassedIndexTypes>
struct index_forward<Derived, ct_ops::index_sequence<AccessIndices...>,
                     PassedIndexTypes...> {
  typedef typename indicial_traits<Derived>::entry_type entry_type;

  static entry_type &work(indicial_base<Derived> &data_owner,
                          std::vector<std::size_t> &dim_offsets,
                          PassedIndexTypes... passed_indices) {
    return data_owner(
        apply_offsets_to_param_pack<
            Derived, AccessIndices,
            std::tuple<PassedIndexTypes...> >::work(dim_offsets,
                                                    passed_indices...)...);
  }
};
...
struct flex_view : indicial_base<flex_view<Derived> > {
  ...
  flex_view(indicial_base<Derived> &data_owner,
            const std::vector<std::size_t> &dim_starts,
            const std::vector<std::size_t> &dim_sizes)
      : data_owner(data_owner), dim_starts(dim_starts), dim_sizes(dim_sizes) {
  ...
  std::size_t dim_size_impl(std::size_t i) const { return dim_sizes[i]; }
  ...
  entry_type &indexing_impl(IndexTypes... indices) {
    if (sizeof...(indices) == 1) {
      ...
      typedef typename ct_ops::make_index_sequence<sizeof...(indices)>::type
          AccessIndices;

      return flex_view_detail::index_forward<Derived, AccessIndices,
                                             IndexTypes...>::work(data_owner,
                                                                  dim_starts,
                                                                  indices...);
    } else {
...
template <class Derived>
flex_view<Derived> make_flex_view(indicial_base<Derived> &data_owner,
                                  const std::vector<std::size_t> &dim_starts,
                                  const std::vector<std::size_t> &dim_sizes) {
