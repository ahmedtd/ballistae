...
struct initializer {
  ...
  static void work(std::vector<ContainerEntry> &container,
                   std::vector<std::size_t> &dimensions,
                   const std::initializer_list<ListEntry> source) {
    initializer<std::initializer_list<ListEntry>>::work_helper(
        container, dimensions, source, true, 0);
  }

  template <class ContainerEntry>
  static std::size_t work_helper(std::vector<ContainerEntry> &container,
                                 std::vector<std::size_t> &dimensions,
                                 const std::initializer_list<ListEntry> source,
                                 bool first_at_level,
...
struct initializer<std::initializer_list<ListEntry>> {
  ...
  static std::size_t work_helper(
      ...
      std::vector<std::size_t> &dimensions,
      const std::initializer_list<std::initializer_list<ListEntry>> source,
      bool first_at_level, std::size_t expected_size) {
    ...
    for (const auto cur_list : source) {
      level_size = initializer<ListEntry>::work_helper(
          container, dimensions, cur_list, first, level_size);
...
struct address {
  ...
  static std::size_t work(DimIt dims_begin, DimIt dims_end,
                          IndicesHeadType indices_head,
                          IndicesTailTypes... indices_tail) {
    result_struct results =
        address<IndicesHeadType, IndicesTailTypes...>::work_helper(
            dims_begin, dims_end, indices_head, indices_tail...);
    return results.offset;
  }

  template <class DimIt>
  static result_struct work_helper(DimIt dims_begin, DimIt dims_end,
                                   IndicesHeadType indices_head,
                                   IndicesTailTypes... indices_tail) {
    result_struct results = address<IndicesTailTypes...>::work_helper(
        dims_begin + 1, dims_end, indices_tail...);
...
struct address<IndicesFinalType> {
  ...
  static std::size_t work(DimIt dims_begin, DimIt dims_end,
    ...
    result_struct results = address<IndicesFinalType>::work_helper(
        dims_begin, dims_end, indices_final);
    return results.offset;
  }
...
struct flex : public indicial_base<flex<Entry>> {
 ...
 public:
  ...
  flex(const flex<Entry> &other)
      : dimensions(other.dimensions), container(other.container) {}

  /// Copy construction (with implicit type conversion).
  template <class OtherEntry>
  flex(const flex<OtherEntry> &other)
      : dimensions(other.dimensions), container(other.container.size()) {
    std::copy(begin(other.container), end(other.container), begin(container));
  }

  /// Move construction (must be same type).
  flex(flex<Entry> &&other)
      : dimensions(move(other.dimensions)), container(move(other.container)) {}
  ...
  flex(DimTypes... dimvalues)
    ...
    ct_ops::argpack_to_range<DimTypes...>::work(begin(dimensions),
                                                dimvalues...);
  }
  ...
  flex(std::initializer_list<MaybeEntryOrList> init) {
    flex_detail::initializer<MaybeEntryOrList>::work(container, dimensions,
                                                     init);
  }

  std::size_t n_dims_impl() const { return dimensions.size(); }

  std::size_t dim_size_impl(const std::size_t which_dim) const {
    return dimensions[which_dim];
  }

  std::size_t n_entries_impl() const { return container.size(); }

  // Both linear and normal access handled by the same machinery.
  template <class... IndexTypes>
  Entry &indexing_impl(IndexTypes... indices) {
    return container[flex_detail::address<IndexTypes...>::work(
        begin(dimensions), end(dimensions), indices...)];
  }
  ...
  flex<Entry> &operator=(const flex<Entry> &source) {
    return crtp::cast<flex<Entry>>(
        assign_detail::assign_helper<
            ...
            indicial_base<flex<Entry>>>::assign(source, *this));
  }
  ...
  auto assign_impl(OtherType &&other) -> decltype(
      assign_detail::assign_helper<decltype(other), decltype(*this)>::assign(
          other, *this)) {
    return assign_detail::assign_helper<decltype(other),
                                        decltype(*this)>::assign(other, *this);
  }

  template <class OtherType>
  auto add_assign_impl(OtherType &&other)
      -> decltype(assign_detail::add_assign_helper<
                  decltype(other), decltype(*this)>::add_assign(other, *this)) {
    return assign_detail::add_assign_helper<decltype(other),
                                            decltype(*this)>::add_assign(other,
                                                                         *this);
  }

  template <class OtherType>
  auto subtract_assign_impl(OtherType &&other)
      -> decltype(assign_detail::subtract_assign_helper<
                  decltype(other), decltype(*this)>::add_assign(other, *this)) {
    return assign_detail::subtract_assign_helper<
        decltype(other), decltype(*this)>::add_assign(other, *this);
  }
