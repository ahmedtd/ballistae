...
TEST(FlagsPathUtilTest, TestInitialProgamName) {
  flags::SetProgramInvocationName("third_party/cc/absl/absl/flags/program_name_test");
  std::string program_name = flags::ProgramInvocationName();
  for (char& c : program_name)
    if (c == '\\') c = '/';

#if !defined(__wasm__) && !defined(__asmjs__)
  const std::string expect_name = "third_party/cc/absl/absl/flags/program_name_test";
...
TEST(FlagsPathUtilTest, TestProgamNameInterfaces) {
  ...
  absl::string_view not_null_terminated("third_party/cc/absl/absl/aaa/bbb");
