...
$(augmk_d)/ballistae_geometry_cylinder.so : d := $(augmk_d)
$(augmk_d)/ballistae_geometry_cylinder.so : | src/libballistae/libballistae.so
$(augmk_d)/ballistae_geometry_cylinder.so : | src/libguile_armadillo/libguile_armadillo.so
$(augmk_d)/ballistae_geometry_cylinder.so : $($(augmk_d)_objs)
	$(CXX) -shared -o $@ $^ $($(d)_CXXFLAGS) $($(d)_LFLAGS) $(CXXFLAGS)

ALL_TARGETS   += $(augmk_d)/ballistae_geometry_cylinder.so
CLEAN_TARGETS += $(augmk_d)/ballistae_geometry_cylinder.so

INSTALL_TARGETS += $(guile (augmk/install-lib-phony "$(augmk_d)_install" "./" "$(augmk_d)/ballistae_geometry_cylinder.so"))
