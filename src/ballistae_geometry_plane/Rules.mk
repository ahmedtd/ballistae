...
$(augmk_d)/ballistae_geometry_plane.so : d := $(augmk_d)
$(augmk_d)/ballistae_geometry_plane.so : | src/libballistae/libballistae.so
$(augmk_d)/ballistae_geometry_plane.so : | src/libguile_armadillo/libguile_armadillo.so
$(augmk_d)/ballistae_geometry_plane.so : $($(augmk_d)_objs)
	$(CXX) -shared -o $@ $^ $($(d)_CXXFLAGS) $($(d)_LFLAGS) $(CXXFLAGS)

ALL_TARGETS += $(augmk_d)/ballistae_geometry_plane.so
CLEAN_TARGETS += $(augmk_d)/ballistae_geometry_plane.so

INSTALL_TARGETS += $(guile (augmk/install-lib-phony "$(augmk_d)_install" "./" "$(augmk_d)/ballistae_geometry_plane.so"))
