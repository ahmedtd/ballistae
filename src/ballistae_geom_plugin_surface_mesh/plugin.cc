...
class surface_mesh_priv : public ballistae::geom_priv
{
...
{
    return tri_mesh_contact(query, mesh, must_overlap, CONTACT_INTO);
}
...
{
    return tri_mesh_contact(query, mesh, must_overlap, CONTACT_EXIT);
}
...
)
{
    ...
    size_t errline;
    tri_mesh the_mesh;
    std::tie(errc, errline, the_mesh) = tri_mesh_load_obj(filename);
    ...
    auto result = std::make_shared<surface_mesh_priv>(std::move(the_mesh));
    return result;
}
