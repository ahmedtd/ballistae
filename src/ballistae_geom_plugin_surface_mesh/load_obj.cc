...
std::tuple<int, size_t, tri_mesh> parse_obj(const char *src, const char *lim)
{
    ...
    while(cur != lim)
    {
        ...
        std::tie(success, cur) = parse_vertex_line(cur, lim, the_mesh);
        if(success)
            continue;
        ...
        std::tie(success, cur) = parse_normal_line(cur, lim, the_mesh);
        if(success)
            continue;

        std::tie(success, cur) = parse_face_line(cur, lim, the_mesh);
        if(success)
            continue;
