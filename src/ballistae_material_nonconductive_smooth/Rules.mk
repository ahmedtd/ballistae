...
$(augmk_d)/ballistae_material_nonconductive_smooth.so : | src/libguile_armadillo/libguile_armadillo.so
$(augmk_d)/ballistae_material_nonconductive_smooth.so : $($(augmk_d)_objs)
	$(CXX) -shared -o $@ $^ $($(d)_CXXFLAGS) $($(d)_LFLAGS) $(CXXFLAGS)

ALL_TARGETS += $(augmk_d)/ballistae_material_nonconductive_smooth.so
CLEAN_TARGETS += $(augmk_d)/ballistae_material_nonconductive_smooth.so

INSTALL_TARGETS += $(guile (augmk/install-lib-phony "$(augmk_d)_install" "./" "$(augmk_d)/ballistae_material_nonconductive_smooth.so"))
