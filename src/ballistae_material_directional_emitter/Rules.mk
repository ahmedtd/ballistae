...
# the plugin library.
$(augmk_d)/ballistae_material_directional_emitter.so : d := $(augmk_d)
$(augmk_d)/ballistae_material_directional_emitter.so : | src/libballistae/libballistae.so
$(augmk_d)/ballistae_material_directional_emitter.so : | src/libguile_ballistae/libguile_ballistae.so
$(augmk_d)/ballistae_material_directional_emitter.so : $($(augmk_d)_objs)
	$(CXX) -shared -o $@ $^ $($(d)_CXXFLAGS) $($(d)_LFLAGS) $(CXXFLAGS)

ALL_TARGETS += $(augmk_d)/ballistae_material_directional_emitter.so
CLEAN_TARGETS += $(augmk_d)/ballistae_material_directional_emitter.so

INSTALL_TARGETS += $(guile (augmk/install-lib-phony "$(augmk_d)_install" "./" "$(augmk_d)/ballistae_material_directional_emitter.so"))
