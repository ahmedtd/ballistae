...
bool begins_with(ItA a_src, ItA a_lim, ItB b_src, ItB b_lim) {
...
std::tuple<bool, const char *> parse_literal(const char *src, const char *lim,
...
std::tuple<bool, const char *, size_t> parse_size_t(const char *src,
                                                    const char *lim) {
  const char *cur = src;
  ...
  if (parsed < std::numeric_limits<size_t>::min() ||
      parsed > std::numeric_limits<size_t>::max() || cur == src)
    return std::make_tuple(false, src, std::numeric_limits<size_t>::max());
  else
    return std::make_tuple(true, cur, parsed);
}

std::tuple<bool, const char *, double> parse_double(const char *src,
                                                    const char *lim) {
  const char *cur = src;
...
std::tuple<bool, const char *> parse_newline(const char *src, const char *lim) {
  ...
  while (cur != lim && (*cur == 0x9 || *cur == 0x20)) ++cur;

  // This will handle pretty much any newline sequence we can expect to see.

  if (cur != lim && *cur == 0xd) ++cur;

  if (cur != lim && *cur == 0xa) ++cur;
...
std::tuple<bool, const char *> parse_blank_line(const char *src,
                                                const char *lim) {
  const char *cur = src;
  ...
  if (!success) return std::make_tuple(false, src);

  return std::make_tuple(true, cur);
}

std::tuple<bool, const char *> parse_comment_line(const char *src,
                                                  const char *lim) {
  const char *cur = src;
  ...
  while (cur != lim && *cur != 0xa && *cur != 0xd) ++cur;
...
std::tuple<bool, const char *> parse_texcoord_line(const char *src,
                                                   const char *lim,
                                                   tri_mesh &the_mesh) {
  const char *cur = src;
...
std::tuple<bool, const char *> parse_normal_line(const char *src,
                                                 const char *lim,
                                                 tri_mesh &the_mesh,
                                                 bool swapyz) {
  const char *cur = src;
...
std::tuple<bool, const char *> parse_vertex_line(const char *src,
                                                 const char *lim,
                                                 tri_mesh &the_mesh,
                                                 bool swapyz) {
  const char *cur = src;
...
std::tuple<bool, const char *, std::array<size_t, 3>> parse_index_triple(
    const char *src, const char *lim) {
  const char *cur = src;

  std::array<size_t, 3> result = {std::numeric_limits<size_t>::max(),
  ...
  if (!success) return std::make_tuple(false, src, result);

  // If don't get a slash next, we're done.
  std::tie(success, cur) = parse_literal(cur, lim, "/");
  if (!success) return std::make_tuple(true, cur, result);
...
std::tuple<bool, const char *> parse_face_line(const char *src, const char *lim,
                                               tri_mesh &the_mesh,
                                               bool swapyz) {
  const char *cur = src;
...
std::tuple<bool, const char *> parse_group_line(const char *src,
                                                const char *lim) {
  bool success;
  const char *cur = src;
...
std::tuple<bool, const char *> parse_mtllib_line(const char *src,
                                                 const char *lim) {
  bool success;
  const char *cur = src;
...
std::tuple<bool, const char *> parse_usemtl_line(const char *src,
                                                 const char *lim) {
  bool success;
  const char *cur = src;
...
std::tuple<int, size_t, tri_mesh> parse_obj(const char *src, const char *lim,
                                            bool swapyz) {
  tri_mesh the_mesh;
  size_t cur_line = 0;
  ...
  for (tri_face_idx &idx : the_mesh.f) {
    for (size_t i = 0; i < 3; ++i) {
      --idx.vi[i];
      if (idx.ni[i] != std::numeric_limits<size_t>::max()) --idx.ni[i];
      if (idx.mi[i] != std::numeric_limits<size_t>::max()) --idx.mi[i];
    }
  }
...
std::tuple<int, size_t, tri_mesh> tri_mesh_load_obj(const std::string &filename,
                                                    bool swapyz) {
  FILE *f = std::fopen(filename.c_str(), "rb");
  if (f == nullptr) {
    return std::make_tuple(OBJ_ERRC_FILE_NOT_OPENABLE, std::size_t(0),
                           tri_mesh());
  }
  ...
  std::tie(errc, error_line, the_mesh) =
      parse_obj(&buf[0], &buf[buf.size()], swapyz);
