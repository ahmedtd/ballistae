...
constexpr size_t fixed_dimension_address(
    IdxHead idxhead,
    list_end_tag unused,
    DimHead dimhead
)
...
template<
    class IdxHead, class IdxSecond, class... IdxTail,
    class DimHead, class DimSecond, class... DimTail
    >
constexpr size_t fixed_dimension_address(
    IdxHead idxhead, IdxSecond idxsecond, IdxTail... idxtail,
    list_end_tag unused,
    DimHead dimhead, DimSecond dimsecond, DimTail... dimtail
)
{
    return idxhead * ct_ops::product(dimsecond, dimtail...)
        + fixed_dimension_address(
            idxsecond,
            idxtail...,
            unused,
            dimsecond,
...
template<
    ...
    >
size_t address(
    IndexIterator index_beg,
    IndexIterator index_end,
    WidthIterator width_beg,
    WidthIterator width_end
)
{
    assert(std::distance(index_beg, index_end) == std::distance(width_beg, width_end));
