...
class image {
 ...
 public:
  image(std::initializer_list<std::size_t> idx_lims_in, Elt fill = Elt(0))
      : idx_lims(idx_lims_in),
        storage(std::accumulate(idx_lims.begin(), idx_lims.end(), 1,
  ...
  image &resize(ArgTail... arg_tail) {
    idx_lims = {arg_tail...};
    storage.resize(std::accumulate(idx_lims.begin(), idx_lims.end(), 1,
                                   [](size_t a, size_t b) { return a * b; }));
  }

  template <class... ArgTail>
  Elt &index(ArgTail... arg_tail) {
    std::array<size_t, sizeof...(arg_tail)> indices = {
        {static_cast<size_t>(arg_tail)...}};
    size_t stride = 1;
    size_t comp_index = 0;
  ...
  size_t n_indices() const { return idx_lims.size(); }

  size_t idx_lim(size_t which_idx) const { return idx_lims[which_idx]; }
};
...
int write_pfm(const image<Elt> &img, const std::string &file) {
  ...
  static_assert(CHAR_BIT == 8, "Assume that a char has 8 bits.");
  ...
  if (fd == -1) goto cleanup_exit_error;

  if (ftruncate(fd, file_size) != 0) goto cleanup_close_file;

  buf = mmap(nullptr, file_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);

  if (buf == MAP_FAILED) goto cleanup_close_file;
  ...
  if (munmap(buf, file_size) == -1) goto cleanup_close_file;
